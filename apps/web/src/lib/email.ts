...
interface SendRegistrationVerificationCodeParams {
  to: string;
  code: string;
}

export async function sendRegistrationVerificationCodeEmail({ to, code }: SendRegistrationVerificationCodeParams) {
  if (!resend) {
    console.log("=================================================");
    console.log("[EMAIL SERVICE] RESEND_API_KEY tidak ditemukan!");
    console.log(`[REGISTRATION VERIFICATION CODE EMAIL FOR ${to}]: ${code}`);
    console.log("Berlaku selama 10 menit.");
    console.log("=================================================");
    return { success: true, simulated: true };
  }
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: EMAIL_FROM,
      to: [to],
      subject: "Kode Verifikasi Pendaftaran Akun EZMON",
      html: `
        <!DOCTYPE html>
        <html>
        <head>
          <meta charset="utf-8">
          <meta name="viewport" content="width=device-width, initial-scale=1.0">
          <title>Kode Verifikasi Pendaftaran EZMON</title>
        </head>
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #09090b; color: #f4f4f5; margin: 0; padding: 40px 20px;">
          <div style="max-width: 500px; margin: 0 auto; background-color: #18181b; border: 1px solid #27272a; border-radius: 12px; padding: 32px; box-shadow: 0 10px 25px rgba(0,0,0,0.5);">
            <div style="text-align: center; margin-bottom: 24px;">
              <h2 style="font-size: 24px; font-weight: 600; color: #ffffff; margin: 0 0 8px 0;">EZMON</h2>
              <p style="font-size: 14px; color: #a1a1aa; margin: 0;">Kode Verifikasi Pendaftaran Akun</p>
            </div>
            
            <p style="font-size: 14px; line-height: 1.6; color: #d4d4d8; margin-bottom: 24px; text-align: center;">
              Gunakan kode verifikasi 6-digit di bawah ini untuk mengonfirmasi pendaftaran akun EZMON Anda. Kode ini berlaku selama <strong>10 menit</strong>.
            </p>

            <div style="text-align: center; margin: 28px 0;">
              <div style="display: inline-block; background-color: #09090b; border: 1px solid #3f3f46; letter-spacing: 6px; font-size: 32px; font-weight: 800; color: #10b981; padding: 16px 36px; border-radius: 10px; font-family: monospace;">
                ${code}
              </div>
            </div>

            <p style="font-size: 12px; line-height: 1.5; color: #71717a; margin-top: 32px; padding-top: 16px; border-top: 1px solid #27272a; text-align: center;">
              Jika Anda tidak merasa melakukan pendaftaran di EZMON, Anda dapat mengabaikan email ini.
            </p>
          </div>
        </body>
        </html>
    ...
    if (error) {
      console.error("[RESEND OTP REGISTRATION ERROR]", error);
      throw new Error(error.message);
    }
